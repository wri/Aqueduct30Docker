...
for sector in sectors:
    for indicator in indicators:
        column_name_weight = "{}_{}_weight".format(indicator.lower(),sector.lower())
        column_name_score = "{}_score".format(indicator.lower())
        column_name_weighted_score = "{}_{}_weightedscore".format(indicator.lower(),sector.lower())
...
def mask_weights():
    ...
    for sector in sectors:
        for indicator in indicators:
            weight_column_name = "{}_{}_weight".format(indicator.lower(),sector.lower())
            score_column_name = "{}_{}_weightedscore".format(indicator.lower(),sector.lower())
...
for sector in sectors:    
    # weights    
    regex_w = '^.*_{}_weight$|^string_id$'.format(sector.lower())
    df_w = df_merged_weights.filter(regex=regex_w)
    df_w = df_w.set_index("string_id")
    column_name_w = "{}_weight_sum".format(sector.lower())
    df_merged_weights[column_name_w] = df_w.sum(axis=1).values
    # scores
    regex_s = '^.*_{}_weightedscore$|^string_id$'.format(sector.lower())
    df_s = df_merged_weights.filter(regex=regex_s)
    df_s = df_s.set_index("string_id")
    column_name_s = "{}_weightedscore_sum".format(sector.lower())
    df_merged_weights[column_name_s] = df_s.sum(axis=1).values
    df_merged_weights["owr_{}_score".format(sector.lower())] = df_merged_weights[column_name_s] / df_merged_weights[column_name_w]
